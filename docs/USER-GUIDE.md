...
- **Default-Trip pinnen (seit v3.0, #227):** Hover → **Pin-Icon** klicken. Der gepinnte Trip wird beim nächsten App-Start automatisch geöffnet — egal welcher Trip zuletzt aktiv war. Cross-device synchronisiert (lebt in `users/{uid}.defaultWorkspaceId`), also gilt dein Pin auch auf iPhone/anderem Gerät. Pin nochmal klicken = entpinnen. Manuelle Trip-Wechsel überschreiben den Pin nicht — nur der nächste App-Start greift wieder den Default.
